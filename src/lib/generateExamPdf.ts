...
import jsPDF from "jspdf";
import { ExamData } from "./types";
import { NotoSansBengaliBase64 } from "./NotoSansBengaliBase64";
...
export function generateExamPdf(data: ExamData): void {
  ...
  // ── Register & set Bangla-compatible font ─────────────────
  ...
  data.questions.forEach((question, index) => {
    ...
    // Question text — always use NotoSansBengali to support both scripts
    doc.setTextColor(30, 30, 30);
    doc.setFontSize(10.5);
    doc.setFont("NotoSansBengali", "normal");
    const lines = doc.splitTextToSize(question, maxTextWidth);
